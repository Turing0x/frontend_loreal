...
class DetailsCollsDebt extends StatefulWidget {
  const DetailsCollsDebt({super.key, required this.id, required this.percent});
...
class ShowList extends StatefulWidget {
  const ShowList({super.key, required this.debtId, required this.percent});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Column(
        ...
        children: [
          Visibility(visible: total != 0, child: columnCals(per)),
          Flexible(
            child: FutureBuilder(
              future: DBProviderTypeCollectiosDebt.db
                  .getAllCollectionsDebtByOwner(widget.debtId),
  ...
  Column columnCals(double per) {
    return Column(
      children: [
        ...
        (pierde > gana)
            ? textoDosis(
                '$total % ${widget.percent} = ${(total * per).toStringAsFixed(2)}',
                20)
            : Padding(
                padding: const EdgeInsets.symmetric(horizontal: 40),
                child: textoDosis(
                    'No hay beneficio, el Gana es mayor que el Pierde', 20,
