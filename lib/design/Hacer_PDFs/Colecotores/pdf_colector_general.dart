...
class PdfInvoiceApiColectorGeneral {
  static Future<Map<String, dynamic>> generate(InvoiceColector invoice,
    ...
    try {
      ...
      for (var object in invoice.usersColector) {
        int gasto = (object.limpio * globalGastoToPDF).toInt();
  ...
  static Widget buildInvoice(InvoiceColector invoice) {
    ...
    final data = invoice.usersColector.map((item) {
      int gasto = (item.limpio * globalGastoToPDF).toInt();
  ...
  static pw.Column topHeader(InvoiceColector invoice) =>
      (globalresumen) ? columnResumen(invoice) : columnCG(invoice);

  static pw.Column columnCG(InvoiceColector invoice) {
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        SizedBox(height: 0.5 * PdfPageFormat.cm),
        pwtextoDosis('VALE DE SALIDA DE COLECTOR GENERAL', 20,
            fontWeight: FontWeight.bold),
